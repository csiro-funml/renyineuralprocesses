...
class ANP(nn.Module):
    ...
    def compute_robust_divergence(self, batch, py, z, pz, qz, num_samples=1):
        ...
        if qz is not None: # L_vi instead of L_ml
            ...
            if self.alpha == 1.0: # KL divergence
                kld = kl_divergence(qz, pz).sum(-1)
                bound = (torch.logsumexp(log_py.sum(-1), dim=0) - math.log(log_w.shape[0]) - kld) / py.loc.shape[
                    -2]  #
                # bound = (torch.logsumexp(log_w, dim=0) - math.log(log_w.shape[0])) / py.loc.shape[
                #     -2]  # devided by n_target
                loss = -bound.mean()  #
            else:  # Renyi alpha divergence
                log_w = (1- self.alpha) * log_w
                bound = (torch.logsumexp(log_w, dim=0) - math.log(log_w.shape[0]))/((1- self.alpha)*py.loc.shape[-2])  # devived by n_target
                loss = -bound.mean() # batch average
        else: #L_ML
            log_w = log_py.sum(-1)
            bound = (torch.logsumexp(log_w, dim=0) - math.log(log_w.shape[0]))/py.loc.shape[-2]
            loss = -bound.mean()
        return loss
    ...
    def forward(self, batch, num_samples=None, reduce_ll=True):
        ...
        if self.training:
            ...
            else: # use the context distribution for sampling
