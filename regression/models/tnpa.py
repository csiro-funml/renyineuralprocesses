...
class TNPA(TNP):
    ...
    def forward(self, batch, reduce_ll=True, num_samples=1):
        z_target, pz, qz = self.encode(batch, autoreg=True, num_samples=num_samples)
        # z_target = self.encode(batch, autoreg=True, num_samples=num_samples)
        ...
        return pred_tar, z_target, pz, qz
        # return pred_tar, outs
    ...
    def permute_sample_batch(self, xt, yt, num_samples, batch_size, num_target):
        ...
        dim_sample = torch.arange(num_samples, device='cuda').unsqueeze(-1).unsqueeze(-1).repeat(
            (1, batch_size, num_target))
        dim_batch = torch.arange(batch_size, device='cuda').unsqueeze(0).unsqueeze(-1).repeat(
            (num_samples, 1, num_target))
        return xt[dim_sample, dim_batch, perm_ids], yt[
            dim_sample, dim_batch, perm_ids], dim_sample, dim_batch, deperm_ids
    ...
    def predict(self, xc, yc, xt, num_samples=50, return_samples=False):
        ...
        if self.permute:
            xt_stacked, yt_stacked, dim_sample, dim_batch, deperm_ids = self.permute_sample_batch(xt_stacked,
                                                                                                  yt_stacked,
                                                                                                  num_samples,
                                                                                                  batch_size,
                                                                                                  num_target)
