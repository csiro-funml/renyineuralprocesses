...
__all__ = ['PoolingEncoder', 'CrossAttnEncoder', 'Decoder', 'SelfAttn']
...
class PoolingEncoder(nn.Module):
    ...
    def __init__(self, dim_x=1, dim_y=1,
        ...
        self.outputz_dim = dim_lat if self.use_lat else dim_hid
        self.net_post = build_mlp(dim_hid, dim_hid,
                2*dim_lat if self.use_lat else dim_hid,
                post_depth)

    def forward(self, xc, yc, mask=None, return_out=False):
            ...
            if self.use_lat:
                ...
                if return_out:
                    return Normal(mu, sigma), out
                return Normal(mu, sigma)
            else:
                if return_out:
                    return self.net_post(out), out  # [B,Eh]
